...
storage_path = '/scratch/sy1743/wildlife/skullsunlimited/'
...
def itemsfromAmazonSERP(url):
    try:
        ...
        h3s = parsed.findAll("div", {"class":"title_common"})
        results = []
        for link in h3s:
            results.append("http://www.skullsunlimited.com"+link.find('a').get('href'))
        return results
    except:
...
def getTitle(parsed):
    try:
        title = parsed.findAll('div')
        return title[0].find('h1').text.strip().replace('\t','').replace('\r','').replace('\n','')
    except:
        return []

def getPrice(parsed):
    try:
        price = parsed.findAll('div', {'class': 'price'})
...
def getDescription(parsed):
    try:
        descs = parsed.findAll('div', {'class': 'overview'})
...
def getImages(parsed):
    imgDict = []
    try:
        m = parsed.findAll('td', {'class':'photo'})
        for i in m:
            imgDict.append("http://www.skullsunlimited.com"+i.find('img').get('src'))
        return imgDict
    except:
        return []
...
def getID(url):
    return urlparse.urlparse(url).query.split('=')[-1] + '_' + str(int(time.time()))
...
def main():
    ...
    url_patt = "http://www.skullsunlimited.com/search_results.php?search_term={}"
