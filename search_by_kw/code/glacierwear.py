...
storage_path = '/scratch/sy1743/wildlife/glacierwear/'
...
def itemsfromAmazonSERP(url):
    try:
        ...
        h3s = parsed.findAll("h2", {"class":"product-name"})
        results = []
        for link in h3s:
...
def getTitle(parsed):
    try:
        title = parsed.findAll('h1', {'itemprop': 'name'})
        return title[0].text
    except:
        return []
...
def getPrice(parsed):
    try:
        price = parsed.findAll('meta', {'itemprop': 'price'})
        return price[0].get('content').strip().replace('\t','').replace('\r','').replace('\n','')
    except:
        return []

def getDescription(parsed):
    try:
        descs = parsed.findAll('div', {'class': 'std'})
...
def getImages(parsed):
    imgDict = []
    try:
        m = parsed.findAll('div', {'class':'product-image-gallery'})
        for i in m:
...
def getShippingInfo(parsed):
    try:
        seller = parsed.findAll('div', {'itemprop':'offers'})
        return seller[0].find('b').text.strip().replace('\t','').replace('\r','').replace('\n','')
    except:
        return []
...
def main():
    ...
    url_patt = "http://www.glacierwear.com/catalogsearch/result/index/?ajaxcatalog=true&dir=desc&limit=24&order=price&q={}"
