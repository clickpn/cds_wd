...
storage_path = '/scratch/sy1743/wildlife/taxidermy_store/'
...
def getTitle(parsed):
    try:
        title = parsed.findAll('div', {'class': 'product-name'})
        return title[0].find('h2').text
    except:
        return []
...
def getDescription(parsed):
    try:
        descs = parsed.findAll('div', {'id': 'product_tabs_description_contents'})
...
def getShippingInfo(parsed):
    try:
        ship = parsed.findAll('div', {'id':'product_tabs_pp_standard_delivery_info_contents'})
        return ship[0].text.strip().replace('\t','').replace('\r','').replace('\n','')
    except:
        return []

def getID(url):
    return urlparse.urlparse(url).path.split('-')[-1].strip('.html') + '_' + str(int(time.time()))
...
def main():
    ...
    url_patt = "http://www.thetaxidermystore.com/catalogsearch/result/index/?cat=0&dir=desc&limit=60&order=sku&q={}"
